...
// 1. Remove Wiki button from the 'More' dropdown
html = html.replace(/<button class="nav-btn" onclick="showSection\('wiki'\)">.*?Wiki<\/button>/g, '');

// 2. Remove the standalone Wiki Section if it exists
const wikiSectionRegex = /<!-- WIKI SECTION -->[\s\S]*?<div id="section-wiki" class="section">([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>/;
// Let's just remove the whole section string by taking what we inserted last time
const standaloneWiki = `<!-- WIKI SECTION -->
...
html = html.replace(standaloneWiki, '');

// 3. Append the Wiki Card strictly AFTER the Data Backup Card
const wikiCard = `
  <!-- WIKI -->
  <div class="card" style="margin-top:24px; border-color:var(--accent)">
    <div class="card-title" style="font-size:1.8rem; margin-bottom:20px; color:var(--accent)">&#128214; Wiki</div>
    <div style="font-size:1.1rem; line-height:1.6; color:var(--text)">
...
if (html.includes(backupHtml) && !html.includes('&#128214; Wiki</div>')) {
  html = html.replace(backupHtml, backupHtml + '\n' + wikiCard);
  fs.writeFileSync('index.html', html, 'utf-8');
  console.log("Wiki firmly placed after Backup!");
} else {
