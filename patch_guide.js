...
// 1. Add to Dropdown Menu
const dropdownRegex = /<button class="nav-btn" onclick="showSection\('reading'\)">.*?Reading Practice<\/button>/;
const guideNav = `<button class="nav-btn" onclick="showSection('reading')">&#128214; Reading Practice</button>
        <button class="nav-btn" onclick="showSection('guide')">&#128216; Study Guide</button>`;

if (!html.includes('showSection(\'guide\')')) {
  html = html.replace(dropdownRegex, guideNav);
}
...
if (!html.includes('section-guide')) {
  html = html.replace('<!-- RESTORED SECTIONS -->', '<!-- RESTORED SECTIONS -->\n' + guideSection);
  fs.writeFileSync('index.html', html, 'utf-8');
}

console.log('Study Guide added to the App!');
