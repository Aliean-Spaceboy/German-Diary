...
// 1. Remove Wiki from Dashboard
const wikiRegex = /<!-- WIKI -->[\s\S]*?<\/div>\s*<\/div>/;
html = html.replace(wikiRegex, '');

// 2. Add Wiki Section back as a standalone section
const wikiSection = `
<!-- WIKI SECTION -->
<div id="section-wiki" class="section">
  <div class="card">
...
if (!html.includes('id="section-wiki"')) {
  html = html.replace('<!-- RESTORED SECTIONS -->', '<!-- RESTORED SECTIONS -->\n' + wikiSection);
}

// 3. Add Wiki button back to the dropdown menu, after Dictation
const dropdownRegex = /<button class="nav-btn" onclick="showSection\('dictation'\)">.*?Dictation<\/button>/;
const readingRegex = /<button class="nav-btn" onclick="showSection\('reading'\)">.*?Reading Practice<\/button>/;

// We need to make sure the dropdown has Reading Practice and Wiki
let dropdownInsert = `<button class="nav-btn" onclick="showSection('dictation')">&#127911; Dictation</button>
        <button class="nav-btn" onclick="showSection('reading')">&#128214; Reading Practice</button>
        <button class="nav-btn" onclick="showSection('wiki')">&#128214; Wiki</button>`;

if (html.includes('showSection(\'reading\')')) {
  // If reading is there, just replace reading with reading + wiki
  html = html.replace(readingRegex, `<button class="nav-btn" onclick="showSection('reading')">&#128214; Reading Practice</button>\n        <button class="nav-btn" onclick="showSection('wiki')">&#128214; Wiki</button>`);
}

fs.writeFileSync('index.html', html, 'utf-8');
console.log('Fixed Wiki successfully!');
