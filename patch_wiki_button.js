...
// 1. Remove the giant Wiki block from the dashboard
const giantWikiRegex = /<!-- WIKI -->[\s\S]*?<\/div>\s*<\/div>/;
html = html.replace(giantWikiRegex, '');

// 2. We need to find the Backup Card and add the button inside/after it
const backupCardEndRegex = /<input type="file" accept="\.json" onchange="importData\(event\)" style="position:absolute; left:0; top:0; opacity:0; cursor:pointer; height:100%">\s*<\/div>\s*<\/div>\s*<\/div>/;

const wikiAccordion = `        <input type="file" accept=".json" onchange="importData(event)" style="position:absolute; left:0; top:0; opacity:0; cursor:pointer; height:100%">
      </div>
    </div>
  </div>
  ...
  <div style="margin-top:14px; text-align:center">
    <button class="btn btn-primary" onclick="toggleWiki()" style="width:100%; font-size:1.1rem">&#128214; Wiki (Study Guide)</button>
  </div>
  
  <div id="wikiContent" style="display:none; margin-top:14px; background:var(--surface); border:1px solid var(--accent); border-radius:12px; padding:20px; text-align:left">
...
html = html.replace(backupCardEndRegex, wikiAccordion);
...
`;
if (!js.includes('function toggleWiki()')) {
  js += '\n' + toggleFunction;
  fs.writeFileSync('script.js', js, 'utf-8');
}

console.log('Wiki button created successfully!');
